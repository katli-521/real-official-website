 ...
 * Timbre: dreamy sine pad — mid-scooped EQ, moderate reverb (0.65 wet), balanced dry signal.
 *
 * Special cases:
 * - NEAR_SILENT_INDICES (Ab2, Ab3): played at very low gain (0.008) — almost unheard.
 * - REST_BASS entries: ghost bass pulse at gain 0.018 — clearly audible but gentle.
 * - Normal melody notes: gain 0.030.
 *
 * Balance:
 * - Mid scoop EQ: peaking filter at 800 Hz, -8 dB, Q=1.2 — removes the "muddy/boxy" mid
 * - Reverb wet: 0.65 (slightly more than before for more space/depth)
 * - Reverb tail: 3.5s
 * - Dry signal: 0.35
 * - Decay: 4.5s
 * - Bass note gain: 0.018
 */
function playNote(audioCtx: AudioContext, muted: boolean) {
  ...
  // Reverb tail — 3.5s, moderate wet mix so notes don't clash
  const convolver = audioCtx.createConvolver();
  const reverbLen = Math.floor(audioCtx.sampleRate * 3.5);
  ...
  wetGain.gain.value = 0.65;  // Slightly more reverb for depth — was 0.55
  convolver.connect(wetGain);
  wetGain.connect(audioCtx.destination);

  // Rest slot — ghost bass, now audible but gentle
  if (freq == null) {
    ...
    bassGain.gain.linearRampToValueAtTime(0.018, now + 0.20);  // Raised from 0.003 — now audible
  ...
  // Determine peak gain: near-silent for Ab2/Ab3 arpeggio notes
  const peakGain = NEAR_SILENT_INDICES.has(noteIndex) ? 0.008 : 0.030;  // Slightly raised

  // Melody note — dreamy pure sine pad (two slightly detuned sines, NO triangle)
  ...
  masterGain.gain.exponentialRampToValueAtTime(0.0001, now + 4.5);  // Shorter decay = less clash

  const osc2Gain = audioCtx.createGain();
  osc2Gain.gain.value = 0.60;

  // Mid-scoop EQ: peaking filter at 800 Hz, -8 dB, Q=1.2
  // Cuts the "boxy/muddy" midrange so notes sound clearer and less congested
  ...
  // Balanced dry/wet — more presence without muddiness
  ...
  midEq.connect(convolver);
  dryGain.connect(audioCtx.destination);

  const stopTime = now + 5.0;
